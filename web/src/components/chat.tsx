...
const Message = ({
    ...
    patterns.forEach(pattern => {
      ...
      while ((match = regex.exec(text)) !== null) {
        allMatches.push({
          ...
          content: pattern.render(match[1], match.index)
        });
      }
    });
  ...
  const parseMarkdownWithHighlights = (line: string, query: string): React.ReactNode => {
  ...
  const highlightTextInString = (text: string, query: string): React.ReactNode => {
